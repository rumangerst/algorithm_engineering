...
int main(int argc, char * argv[])
{
    ...
    // Monitor the time
    const int time_limit __attribute__((aligned(MEMORY_ALIGNMENT))) = stoi(argv[1]);
    ...
    #pragma omp parallel for schedule(static, 1) firstprivate(time_start,time_limit) shared(discussions,table) default(none)
    for(int i = 0; i < N; ++i)
    {
